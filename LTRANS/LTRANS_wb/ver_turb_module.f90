...
! A random displacement model is implemented to simulate sub-grid scale turbulent 
! particle motion in the vertical (z) direction. 
!
...
!    ** simulting displacement due to turbulent diffusion (vertical direction)
!    **    z(t+1)= z + K'(z)dt + R{ 2/r K[z+0.5K'(z)dt]dt}**0.5
!    **    where z = particle vertical location at time t
!    **      K' = dK/dz (Kprime) and K = vertical diffusivity (KH from QUODDY)
...
!    ************************************************************************
  ...
  SUBROUTINE VTurb(P_zc,P_depth,P_zetac,p,ex,ix,Pwc_wzb,Pwc_wzc,Pwc_wzf,TurbV)
	...
	INTEGER, INTENT(IN) :: p
	DOUBLE PRECISION, INTENT(IN) :: P_zc,P_depth,P_zetac,ex(3),ix(3),Pwc_wzb(ws),Pwc_wzc(ws),Pwc_wzf(ws)
	DOUBLE PRECISION, INTENT(OUT) :: TurbV

	DOUBLE PRECISION, PARAMETER:: background = 1.0E-6  !Background vertical diffusivity from ROMS  

	REAL :: deltat,DEV,r
	INTEGER :: i,j,k,jlo,loop
	DOUBLE PRECISION :: slopem,ParZc,Kprimec,KprimeZc,newZc,KH3rdc,Z3rdc,thisyc,ey(3),        &
      Pwc_KHb(ws),Pwc_KHc(ws),Pwc_KHf(ws),slopekb(ws-1),slopekc(ws-1),slopekf(ws-1),          &
      interceptb(ws-1),interceptc(ws-1),interceptf(ws-1),                                     &
	  movexb(p2+1),moveyb(p2+1),movexc(p2+1),moveyc(p2+1),movexf(p2+1),moveyf(p2+1),          &
	  ifitxb(p2),ifityb(p2),ifitxc(p2),ifityc(p2),ifitxf(p2),ifityf(p2),ifitx(p2),ifity(p2),  &
	  newxb(p2+7),newyb(p2+7),newxc(p2+7),newyc(p2+7),newxf(p2+7),newyf(p2+7)

    !TSPACK Variables
    INTEGER :: IER,SigErr
    DOUBLE PRECISION :: YPKc(p2),SIGMAKc(p2)
...
!			*********************************************************
	...
	newxf=0.0
	do j=1,p2+7
	  newxb(j)=Pwc_wzb(1)+(float(j-4))*(Pwc_wzb(ws)-Pwc_wzb(1))/p2
	  newxc(j)=Pwc_wzc(1)+(float(j-4))*(Pwc_wzc(ws)-Pwc_wzc(1))/p2
	  newxf(j)=Pwc_wzf(1)+(float(j-4))*(Pwc_wzf(ws)-Pwc_wzf(1))/p2
	enddo
...
!			*********************************************************
	...
	!ii. take 8-point moving average of proliferated values
	do i=2,p2-1
	  moveyb(i)=(newyb(i)+newyb(i+1)+newyb(i+2)+newyb(i+3)+newyb(i+4)+newyb(i+5)+newyb(i+6)+newyb(i+7))/8.	  
	  movexb(i)=newxb(i)+ (newxb(i+7)-newxb(i))/2.
	  moveyc(i)=(newyc(i)+newyc(i+1)+newyc(i+2)+newyc(i+3)+newyc(i+4)+newyc(i+5)+newyc(i+6)+newyc(i+7))/8.	  
	  movexc(i)=newxc(i)+ (newxc(i+7)-newxc(i))/2.
	  moveyf(i)=(newyf(i)+newyf(i+1)+newyf(i+2)+newyf(i+3)+newyf(i+4)+newyf(i+5)+newyf(i+6)+newyf(i+7))/8.	  
	  movexf(i)=newxf(i)+ (newxf(i+7)-newxf(i))/2.
	enddo

	!iii. fix end points of series to true data values
	movexb(1)= Pwc_wzb(1)
	moveyb(1)= Pwc_KHb(1) 
...
!			*********************************************************
	...
	!iv. fit polynomical and find internal time step b,c,f values at each depth
	...
	!ix. Calculate one profile of vertical diffusivity using R-K 4th order in time. 
	do k=1,p2
	  ifity(k)=( ifityb(k) + 4.0*ifityc(k) + ifityf(k) )/6.0
	  ifitx(k)=( ifitxb(k) + 4.0*ifitxc(k) + ifitxf(k) )/6.0
	enddo

	!v. fit a tension spline to water column profile of KH using TSPACK
	SigErr=0
	CALL TSPSI (p2,ifitx,ifity,YPKc,SIGMAKc,IER,SigErr)

	!vi. Initialize. Set deltat, number of iterations, initial z-coordinate
	deltat=2.0  ! dt= 1 sec
...
!			*********************************************************
	...
	!vii. Begin iterations
	do i=1,loop   
	!  a. Determine the second term in the equation: K'(z)deltat
	!    1.Find Kprime & solve for second term in RDM equation (K'(z)deltat = KprimeZ)
	  Kprimec=0.0
	  KprimeZc=0.0
	  ...
	  KprimeZc=-1*Kprimec*deltat    !need minus sign to switch axes   

	!  b. Determine the 3rd term in the RDM equation: R{ 2/r K[z+0.5K'(z)dt]dt}**0.5
	!    i. Find K at location of [z+0.5K'(z)dt] = Z3rd
	!      1. calculate Z3rd and make sure within boudaries
	  Z3rdc = ParZc + 0.5*KprimeZc
	...
	!  c. Solve the entire equation
	  ...
	  newZc = ParZc + KprimeZc + DEV* (2./r * KH3rdc*deltat)**0.5

	!vii. update particle z-coordinate
	  ParZc = newZc
...
!			*********************************************************
	...
	!viii. find vertical displacement of particle due to turbulent diffusion
	TurbV = P_zc - ParZc
