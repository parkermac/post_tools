...
!*****************************************************************
  ...
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION (:) :: hid,hx,hy,bx,by
...
!		part of, this is continued until all the boundary points are used
  ...
  REAL :: x_u(ui,uj),y_u(ui,uj),x_v(vi,vj),y_v(vi,vj)
        ...
        if(ele(i,j)%form == 6) then
          ...
          if(i==1 .OR. j==uj-1 .OR. i==vi-1 .OR. j==1)then
            ...
            do
              ...
              if(ele(ipos1,jpos1)%form==6 .OR. ele(ipos1,jpos1)%form==9) deadend1 = .true.

              !if the right path has hit a dead end, switch deadend2 to TRUE
              if(ele(ipos2,jpos2)%form==6 .OR. ele(ipos2,jpos2)%form==9) deadend2 = .true.
              ...
              if(.NOT. deadend1) CALL getNext(ipos1,jpos1,wf1,dir1,ele(ipos1,jpos1)%form)

              !if right path has not hit dead end, move to next point on path
              if(.NOT. deadend2) CALL getNext(ipos2,jpos2,wf2,dir2,ele(ipos2,jpos2)%form)
  ...
  do i=2,vi-2
    ...
    do j=2,uj-2
      if(ele(i,j)%form > 0 .AND. ele(i,j)%form < 15)then
        ipos = i				!if and element that isn't all water or all
...
!  write(*,*) ' '
  ...
  do
    ...
    do
      ...
      if(polydone)exit			!if the current island has been finished, exit
...
! land boundary between rho points where mask_rho = 0 and mask_rho = 1.
  ...
  if(numislands>0)then
    do k=1,numislands
      maxisland = maxisland + polysizes(1+k)     !Sum up the total # of island boundary points
    enddo

    ALLOCATE(hid(maxisland))      !  and allocate hid,hx,hy to the # of island boundary points
  ...
  do m=1,numpoly
    ...
    do k=pstart,pend
      ...
      if(m == 1) then					!if it is the main body of water boundary
        ...
        hid(c) = m+999.                 !  add the island # to hid (island numbers start with 1001)
        if( bnds(k)%onU ) then          !  if its on the U grid
    ...
    !At the end of each polygon, put the first boundary point on the end to close the polygon
    c = c + 1
    k = pstart
    i = bnds(k)%ii
    j = bnds(k)%jj
  ...
  ! The bnd_x/bnd_y matrices are used to determine 1) if a particle intersects a boundary 
  ! segment and 2) reflect the particle off the boundary segment.
  nbounds = maxbound + maxisland - numislands - 1
  ALLOCATE(bnd_x(2,nbounds))
  ALLOCATE(bnd_y(2,nbounds))

  ! First create the line segments for the land and open ocean boundaries by simply 
  !     writing each x/y location and the next x/y location. The fact that the first bx/by
  !     coordinate equals the last ensures that all line segments surrounding the model
  !     are included.
  do i=1,maxbound-1
    bnd_x(1,i) = bx(i)   
    bnd_y(1,i) = by(i)
  ...
  if(numislands>0)then
    ! Now create the line segments for the island boundaries. Same principle
    !     as above, but need to account for the fact that there are 4 islands in the
...
!	jjj - j position on the grid
  ...
  if( (polystart /= (b+1)) .AND. (bnds(polystart)%onU .eqv. isU) .AND.       &
...
END SUBROUTINE getNext
  ...
  ! centriod is inside the model domain 
  ! It returns the value 1 in the variable inbounds if the centroid is in bounds
  !   and 0 if it is outside the boundaries
...
!	  write(*,*) blatlon(i,1), " ", blatlon(i,2)
  ...
  SUBROUTINE ibounds(in_island,claty,clongx,island)
    ...
    if(numislands>0)then
      ...
      do
        ...
        if(i == maxisland .OR. hid(i+1) /= isle)then	!if the end of an island has been reached:
          count = i - start								!  set count to the number of points around the island
          ALLOCATE(isbnds(count,2))						!  allocate isbnds
          do j=1,count									!  iterate through the island points reading
            isbnds(j,1) = hx(start+j)					!    the boundary locations into isbnds
            isbnds(j,2) = hy(start+j)
          enddo
          if(INPOLY(clongx,claty,count,isbnds))then		!  call INPOLY to see if the point is in the island
            in_island = 1								!    if so set in_island to 1
            island = isle								!    and set island to the id of the island it is in
            DEALLOCATE(isbnds)							!    deallocate isbnds
            exit										!    and exit the subroutine
          endif
          DEALLOCATE(isbnds)							!  if not in the island, deallocate isbnds
          if(i == maxisland)exit						!  if it was the last island exit the subroutine
          start = i										!  if not, set start to the start point of next island
          isle = hid(i+1)								!  and set isle to the next island id
        endif											!control returns to the start of the loop to 
      enddo												!iterate through the remaining island boundary points
  ...
  subroutine intersect_reflect(Xpos,Ypos,nXpos,nYpos,fintersectX,fintersectY,                 &
	...
	DOUBLE PRECISION :: crossk,dPBC,mBCperp,rx1,rx2,ry1,ry2,Bp,distBC,dist1,dist2,            &
	  intersctx,interscty,rPxyzX,rPxyzY,Mbc,Bbc,Mp,bcx1,bcy1,bcx2,bcy2,bBCperp,               &
	  xhigh,xlow,yhigh,ylow,d_Pinter,dtest,bxhigh,bxlow,byhigh,bylow
	...
	do i=1,nbounds
	  ...
	  if (i == skipbound) cycle
		...
		!If the boundary segment end points are both east, west, north, or south of the
		!  particle's previous or new location, cycle to next boundary
		if( ((bcx1 > xhigh) .AND. (bcx2 > xhigh)) .OR. ((bcx1 < xlow) .AND. (bcx2 < xlow)) .OR.     &
            ((bcy1 > yhigh) .AND. (bcy2 > yhigh)) .OR. ((bcy1 < ylow) .AND. (bcy2 < ylow)) ) cycle
		...
		if (bcx1.EQ.bcx2 .OR. nXpos.EQ.Xpos ) then
		  !test if they are both verticle, if so cycle because they cannot intersect
		  ...
		  elseif (nXpos.EQ.Xpos .AND. bcy1.NE.bcy2  ) then
			...
			if (intersctx.LE.xhigh .AND.  intersctx.GE.xlow .AND.									&
				...
				intersctx.LE.bxhigh .AND. intersctx.GE.bxlow .AND.									&
				interscty.LE.byhigh .AND. interscty.GE.bylow ) then
			  !Now use cross product to determine the distance of the particle from the boundary
			  ...
			  rx1 = sqrt( ((2*dPBC)**2)/(1+mBCperp**2) ) + nXpos
			  ry1 = mBCperp*rx1 + bBCperp
			  rx2 = -1*sqrt( ((2*dPBC)**2)/(1+mBCperp**2) ) + nXpos
			  ry2 = mBCperp*rx2 + bBCperp
			  !point closest to intersection of boundary and particle trajectory is the right one
			  dist1 = sqrt( (intersctx-rx1)**2 + (interscty-ry1)**2 )
			  dist2 = sqrt( (intersctx-rx2)**2 + (interscty-ry2)**2 )
			  if(dist1.LT.dist2) then
				rPxyzX= rx1
		  ...
		  if(intersect == 0)then
			...
			if (Mbc.EQ.0.0) interscty = byhigh !when bc || w/x-axis, byhigh=bylow=intersecty
...
!  boundaries in meters
  ...
  open(1,FILE='xybounds.bln',STATUS='REPLACE')
    ...
    do m=1,numpoly						!iterate through the polygons
      write(1,1) polysizes(m)+1,1		!write polysize,1 for .bln first row
      ...
      do k=pstart,pend					!iterate through polygon bnds
        ...
        if( bnds(k)%onU ) then			!if on U Grid
          write(1,2) x_u(i,j),y_u(i,j)	!  write x & y of U node
        else							!else
          write(1,2) x_v(i,j),y_v(i,j)	!  write x & y of V node
        endif
      enddo
      ...
      j = bnds(k)%jj
      if( bnds(k)%onU ) then
        write(1,2) x_u(i,j),y_u(i,j)
      else
        write(1,2) x_v(i,j),y_v(i,j)
      endif
  ...
  open(1,FILE='llbounds.bln',STATUS='REPLACE')
    ...
    do m=1,numpoly						!iterate through the polygons
      ...
      do k=pstart,pend					!iterate through polygon bnds
        ...
        j = bnds(k)%jj					!j position
        if( bnds(k)%onU ) then                        !if on U Grid
          write(1,2) x2lon(x_u(i,j)),y2lat(y_u(i,j))  !  write lon & lat of U node
        else                                          !else
          write(1,2) x2lon(x_v(i,j)),y2lat(y_v(i,j))  !  write lon & lat of V node
        endif
      enddo
      k = pstart						!go back to polygon's first bnd
      i = bnds(k)%ii					!  to close the polygon
      j = bnds(k)%jj
      if( bnds(k)%onU ) then
        write(1,2) x2lon(x_u(i,j)),y2lat(y_u(i,j))
      else
        write(1,2) x2lon(x_v(i,j)),y2lat(y_v(i,j))
      endif
