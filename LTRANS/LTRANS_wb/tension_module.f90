...
!
    ...
    !DOUBLE PRECISION STORE, TOL, DSMAX
    DOUBLE PRECISION TOL, DSMAX
    LOGICAL CONT,FLAG               !Added by Zachary Schlag

! Initialize values to 'No errors encountered.'
...
!   R is in (0,1) and well-defined iff HPP(X1)*HPP(X2) < 0.
            ...
            F = FMAX
            CONT = .TRUE.
            IF (C1*(SIG*SCM*D1 - SSM*D1PD2) .LT. 0.D0)     &
              CONT = .FALSE.
...
!***********************************************************
    ...
    DATA IL/1/
    TT = T
    IF (X(IL) .GT. TT  .OR.  TT .GE. X(IL+1) .OR.          &
      IL .LT. 1  .OR.  IL .GE. N) THEN
