...
CONTAINS
  ...
  DOUBLE PRECISION, INTENT(in) :: x,y,e(n,2)	!particle x/y location and polygon edge coordinates  
  ...
  do i=1,n
	if( e(i,2) > y ) hilo(i)=1					!if edge point is above ray, set hilo to 1
	if( e(i,2) < y ) hilo(i)=-1					!if edge point is below ray, set hilo to -1
	if( e(i,2) == y .and. e(i,1)>x) on=.true.	!if edge point on ray, set on true
  ...
  if(on)then
	...
	do
	  ...
	  if(hilo(i)==0 .and. e(i,1)>x)then			!if on ray:
		...
		!if j<0 then that means it has iterated past the last boundary point and
		!  cycled back to the beginning, therefore all the boundaries have been
		!  checked and the loop can be exited
		if( j<0 ) exit

		!if it hasn't passed the last boundary point, move to the next one not on the ray
		i=i+j
	  endif
  ...
  do i=1,n-1
	...
	!if it reaches the following code then one boundary point is above, one is below
	!  and one is to the right of the point, while the other is left of the point
	!  therefore it may or may not cross the ray, so the slope and intercept must
	!  be calculated, then the intersection point of the ray with the boundary line
	!  must be calculated using the y coordinate of the point:

	m=  (e(i+1,2)-e(i,2))/(e(i+1,1)-e(i,1))	!slope
	b=  e(i,2)-m*e(i,1)						!intercept
	ix= (y-b)/m								!ix = ray and boundary line intersection point

	if(ix==x)then							!if x coordinate is on the intersection point:
	  if(onout)inpoly=.false.				! if on means out set inpoly false
	  return								! return from the function
	endif

	!if x intersect is to the right of the point, then ray crosses boundary line,
	! therefore increment the variable crossed
