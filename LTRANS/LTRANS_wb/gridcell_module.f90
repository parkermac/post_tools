...
!  The Gridcell Module contains the subroutine gridcell which is used for determining 
!   if a point lies within an element.  Subroutine gridcell serves two purposes.  When
!   passed an element number through the optional argument checkele, it determines if 
!   a particle is in that particular element.  When the optional argument is omitted, 
!   the subroutine determines in which element the particle is currently located.
!
...
CONTAINS
  ...
  SUBROUTINE gridcell(elements,ele_y,ele_x,Xpos,Ypos,P_ele,triangle,checkele)
	...
	DOUBLE PRECISION, INTENT(IN) :: ele_x(4,elements),ele_y(4,elements),Xpos,Ypos
    ...
    if( present(checkele) )then		!if the variable checkele is present:
      ...
      eleend = checkele             !  so that checkele is the only element checked
    else                            !else:
      elestart = 1                  !  initialize elestart to 1 and eleend to elements
      eleend = elements             !  so that all the elements are checked
    endif
	...
	do i=elestart,eleend
      ...
      if ( (Ypos.LT.ele_y(1,i) .AND. Ypos.LT.ele_y(2,i) .AND.										&
		   ...
		   (Ypos.GT.ele_y(1,i) .AND. Ypos.GT.ele_y(2,i) .AND.										&
			Ypos.GT.ele_y(3,i) .AND. Ypos.GT.ele_y(4,i) ) ) cycle	!particle not in element
	  ...
	  if ( (Xpos.LT.ele_x(1,i) .AND. Xpos.LT.ele_x(2,i) .AND.										&
		   ...
		   (Xpos.GT.ele_x(1,i) .AND. Xpos.GT.ele_x(2,i) .AND.										&
			Xpos.GT.ele_x(3,i) .AND. Xpos.GT.ele_x(4,i) ) ) cycle	!particle not in element
	  ...
	  ! 4 check if there is a horizontal boundary segment and if particle is on it
	  if (ele_y(1,i).EQ.ele_y(2,i) .OR. ele_y(1,i).EQ.ele_y(3,i) .OR.								&
		...
		if (ele_y(1,i).EQ.ele_y(2,i) .AND. Ypos.EQ.ele_y(1,i) ) then
		  if( ( ele_x(1,i).GT.ele_x(2,i) .AND. Xpos.GT.ele_x(2,i) .AND. Xpos.LT.ele_x(1,i) ) .OR.	&
			  ( ele_x(2,i).GT.ele_x(1,i) .AND. Xpos.GT.ele_x(1,i) .AND. Xpos.LT.ele_x(2,i) ) ) then
		...
		if (ele_y(1,i).EQ.ele_y(3,i) .AND. Ypos.EQ.ele_y(1,i) ) then
		  if( ( ele_x(1,i).GT.ele_x(3,i) .AND. Xpos.GT.ele_x(3,i) .AND. Xpos.LT.ele_x(1,i) ) .OR.	&
			  ( ele_x(3,i).GT.ele_x(1,i) .AND. Xpos.GT.ele_x(1,i) .AND. Xpos.LT.ele_x(3,i) ) ) then
			P_ele = i   !particle on boundary segement
			triangle = 1
			exit
		  else
		...
		if (ele_y(1,i).EQ.ele_y(4,i) .AND. Ypos.EQ.ele_y(1,i) ) then
		  if( ( ele_x(1,i).GT.ele_x(4,i) .AND. Xpos.GT.ele_x(4,i) .AND. Xpos.LT.ele_x(1,i) ) .OR.	&
			  ( ele_x(4,i).GT.ele_x(1,i) .AND. Xpos.GT.ele_x(1,i) .AND. Xpos.LT.ele_x(4,i) ) ) then
			P_ele = i   !particle on boundary segement
			triangle = 1
			exit
		  else
		...
		if (ele_y(2,i).EQ.ele_y(3,i) .AND. Ypos.EQ.ele_y(2,i) ) then
		  if( ( ele_x(2,i).GT.ele_x(3,i) .AND. Xpos.GT.ele_x(3,i) .AND. Xpos.LT.ele_x(2,i) ) .OR.	&
			  ( ele_x(3,i).GT.ele_x(2,i) .AND. Xpos.GT.ele_x(2,i) .AND. Xpos.LT.ele_x(3,i) ) ) then
			P_ele = i   !particle on boundary segement
			triangle = 1
			exit
		  else
		...
		if (ele_y(2,i).EQ.ele_y(4,i) .AND. Ypos.EQ.ele_y(2,i) ) then
		  if( ( ele_x(2,i).GT.ele_x(4,i) .AND. Xpos.GT.ele_x(4,i) .AND. Xpos.LT.ele_x(2,i) ) .OR.	&
			  ( ele_x(4,i).GT.ele_x(2,i) .AND. Xpos.GT.ele_x(2,i) .AND. Xpos.LT.ele_x(4,i) ) ) then
			P_ele = i   !particle on boundary segement
			triangle = 1
			exit
		  else
		...
		if (ele_y(3,i).EQ.ele_y(4,i) .AND. Ypos.EQ.ele_y(3,i) ) then
		  if( ( ele_x(3,i).GT.ele_x(4,i) .AND. Xpos.GT.ele_x(4,i) .AND. Xpos.LT.ele_x(3,i) ) .OR.	&
			  ( ele_x(4,i).GT.ele_x(3,i) .AND. Xpos.GT.ele_x(3,i) .AND. Xpos.LT.ele_x(4,i) ) ) then
			P_ele = i   !particle on boundary segement
			triangle = 1
			exit
		  else
